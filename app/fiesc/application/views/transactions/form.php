							...
							<?php } ?>
								...
								<?php } ?>
									...
									<div class="form-group">
										...
										<div class="col-md-5">
											 <select name="pessoa_id" class="form-control" onchange="buscar_conta(this)" required >
											 	<option value="0" >  Selecione ..</option>
											 	<?php foreach ($pessoa_cpf as $key => $p) { ?>
											 	<option value="<?=$p->id?>" > <?=$p->pessoa_cat ?></option>
											 <?php } ?>
											 </select>
										</div>
									</div>
									...
									<div class="form-group">
										 <label class="control-label col-md-3">Número da conta  </label>
										<div class="col-md-5" id="div-conta-ajx">
										...
										<div class="col-md-5">
											<input value="<?php if (isset($conteudos[0]->valor)) echo $conteudos[0]->valor; ?>" type="text" name="valor" data-required="1" class="form-control"  />
										</div>
									</div>


									<div class="form-group">
										<label class="control-label col-md-3">Depositar/Retirar
										</label>
										<div class="col-md-5">
											 <select name="tipo_trans" class="form-control"  >
											 	<?php foreach ($tipo_tran as $key => $t) { ?>
...
function buscar_conta(post_id) {

	//alert(post_id.value)
	var pessoa_cod=post_id.value;

	$.ajax({
		type:'POST',
		url: "<?=base_url('getoptions')?>",
		data: {pessoa_id:pessoa_cod},
		beforeSend: function(){
		$("#divirest").show();
		},
			...
			if (result.status == 'success') {
			   $("#div-conta-ajx").html(result.options);
			}

			},
			error: function(data)
			{
			alert('erro ao ao buscar contas!');
			}
	});
